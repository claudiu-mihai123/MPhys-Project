...
def analysis(input_file,det_output_csv,det_output_plot,part_output_plot,part_output_csv):
    ...
    def extract_single_element(x):
            if isinstance(x, list) or isinstance(x,np.ndarray) or isinstance(x,awkward.highlevel.Array):
                if len(x) == 1:
                    ...
                    return np.array(x)
            elif isinstance(x, (np.float32, np.float64, float,int,np.int32)): 
                return x
            else:
                return np.nan  # If
    ...
    #print(events_df.loc[i,'Muon_size'])
        ...
        if events_df.loc[i,'Photon_size']>1:
            ...
            for j in range(len(candidate_photons)):
                temp=np.sqrt((eta_l-events_df.loc[i,'Photon.Eta'][j])**2+(phi_l-events_df.loc[i,'Photon.Phi'][j])**2)
                if temp<0.4:
                    candidate_photons=np.delete(candidate_photons,np.where(candidate_photons==j)[0])
    ...
    for i in CMS_cut.index:
        ...
        if CMS_cut['Jet_size'][i]==0:
            continue
        else:
            if CMS_cut['Jet_size'][i]==1:
            ...
            candidate_jets=np.arange(CMS_cut['Jet_size'][i])
            counted_jets=[]
            for jet in candidate_jets:
                if CMS_cut['Electron_size'][i]==1:
                    dR_l=np.sqrt( (CMS_cut['Electron.Phi'][i]-jet_phi[jet])**2+ (CMS_cut['Electron.Eta'][i]-jet_eta[jet])**2  )
                else:
                    dR_l=np.sqrt( (CMS_cut['Muon.Phi'][i]-jet_phi[jet])**2+ (CMS_cut['Muon.Eta'][i]-jet_eta[jet])**2  )
                dR_y=np.sqrt( (CMS_cut['Photon.Phi'][i]-jet_phi[jet])**2+ (CMS_cut['Photon.Eta'][i]-jet_eta[jet])**2  )
                if (jet_pt[jet]>30) and (np.abs(jet_eta[jet])<2.5) and ( dR_l>0.4 ) and (dR_y>0.4):
                    counted_jets.append(jet)
                else:
                    continue


        
            if len(counted_jets)>0:
                CMS_cut.at[i,'Jet.PT']=jet_pt[counted_jets]
                CMS_cut.at[i,'Jet.Eta']=jet_eta[counted_jets]
                CMS_cut.at[i,'Jet.Phi']=jet_phi[counted_jets]
                CMS_cut.loc[i,'Jet_size']=len(counted_jets)
            else:
                CMS_cut.at[i,'Jet_size']=0
                CMS_cut.at[i,'Jet.PT']=0
                CMS_cut.at[i,'Jet.Eta']=0
                CMS_cut.at[i,'Jet.Phi']=0
    ...
    CROSS=1.15*1000
    W_CONSTANT=138*CROSS/np.sum(events_df_full['Event.Weight'])
    ...
    CMS_cut=CMS_cut_part_initial.loc[(CMS_cut_part_initial['MissingET.MET']>40) &(CMS_cut_part_initial['Photon.PT']>30) & (np.abs(CMS_cut_part_initial['Photon.Eta'])<2.5)&
            
            ( (CMS_cut_part_initial['Electron.PT']>35) | (CMS_cut_part_initial['Muon.PT']>30) ) 
            &
            (  (    (np.abs(CMS_cut_part_initial['Electron.Eta'])<1.44) | (np.abs(CMS_cut_part_initial['Electron.Eta'])>1.57)      &  
                        (np.abs(CMS_cut_part_initial['Electron.Eta'])<2.5)    )          | (np.abs(CMS_cut_part_initial['Muon.Eta'])<2.4)  )  
                
                &(    (  np.sqrt((CMS_cut_part_initial['Electron.Eta'] - CMS_cut_part_initial['Photon.Eta'])**2 +(CMS_cut_part_initial['Electron.Phi'] -CMS_cut_part_initial['Photon.Phi'] )**2  )>0.7
                        )  |
                        ( np.sqrt((CMS_cut_part_initial['Muon.Eta'] - CMS_cut_part_initial['Photon.Eta'])**2 +(CMS_cut_part_initial['Muon.Phi'] -CMS_cut_part_initial['Photon.Phi'] )**2  )>0.7)       )
                ].copy()
    ...
    for i in CMS_cut.index:
        ...
        if CMS_cut['Jet_size'][i]==0:
            ...
            if len(counted_jets)>0:
                ...
                CMS_cut.at[i,'Jet.Eta']=jet_eta[counted_jets]
                CMS_cut.at[i,'Jet.Phi']=jet_phi[counted_jets]
                CMS_cut.at[i,'Jet_size']=len(counted_jets)
            else:
                CMS_cut.at[i,'Jet_size']=0
                CMS_cut.at[i,'Jet.PT']=0
                CMS_cut.at[i,'Jet.Eta']=0
